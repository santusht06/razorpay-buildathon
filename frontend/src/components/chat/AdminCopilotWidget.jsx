...
export const AdminCopilotWidget = ({ currentCaseId, onNavigateToCase }) => {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'assistant',
      content: `👋 **Welcome to Razorpay Recovery Copilot!**\n\nI am your real-time **RAG-powered AI Assistant** for the admin portal. Ask me anything about:\n- 📊 **Revenue Analytics** & recovery rates\n- 🔍 **Specific Cases** (e.g., *“Why was case rc_... escalated?”*)\n- 🛡️ **Merchant Policies** & guardrails\n- ⚡ **Direct Actions** (e.g., *“Approve case rc_...”*)`,
      ...
      {isOpen && (
        <div
          className={`fixed z-50 bg-white border border-slate-200 rounded-3xl shadow-2xl flex flex-col transition-all duration-200 overflow-hidden ${
            isExpanded
              ? 'inset-6 sm:inset-12'
              : 'bottom-6 right-6 w-[92vw] sm:w-[460px] h-[640px] max-h-[88vh]'
          }`}
        >
          {/* Header */}
            ...
            <div className="flex items-center space-x-2.5">
              ...
              <div>
                ...
                <p className="text-[10px] text-slate-400 font-medium">
                  {currentCaseId ? `Focusing on case ${currentCaseId}` : 'Connected to live database & merchant policies'}
                </p>
              </div>
            </div>
          ...
          <div className="flex-1 p-4 overflow-y-auto space-y-4 bg-slate-50/50 text-xs">
                ...
                >
                  ...
                  {msg.sources && msg.sources.length > 0 && (
                    <div className="pt-2 border-t border-slate-100 flex items-center gap-1.5 flex-wrap">
                      <span className="text-[10px] font-extrabold text-slate-400 uppercase">Grounding:</span>
                  ...
                  {msg.suggested_prompts && msg.suggested_prompts.length > 0 && idx === messages.length - 1 && (
                    <div className="pt-2 border-t border-slate-100 space-y-1.5">
                      <div className="text-[10px] font-black text-slate-400 uppercase tracking-wider">Suggested Questions:</div>
                      <div className="flex flex-col gap-1">
                        {msg.suggested_prompts.map((prompt, pIdx) => (
                          <button
                            key={pIdx}
                            onClick={() => handleSendMessage(prompt)}
                            className="text-left text-[11px] text-[#0C54EA] hover:text-[#0A47C4] hover:bg-blue-50/70 p-1.5 rounded-lg border border-blue-100 transition-colors flex items-center justify-between font-bold bg-white"
                          >
                            <span>{prompt}</span>
            ...
            {loading && (
              <div className="flex items-center space-x-2 text-slate-500 text-xs p-2 bg-white rounded-xl border border-slate-200 w-fit">
          ...
          >
            <input
              ...
              placeholder="Ask about recovery stats, case rc_..., or policies..."
              disabled={loading}
