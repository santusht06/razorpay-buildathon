...
export const Dashboard = ({ onSelectCase, onOpenSimulator }) => {
  ...
  return (
    <div className="space-y-6 pb-16">
      ...
      {/* Overview Header Row (from screenshot) */}
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-2">
          <h2 className="text-sm font-bold text-slate-900">Overview</h2>
          <button className="flex items-center space-x-1 text-xs font-bold text-blue-600 hover:text-blue-700 bg-blue-50 px-2 py-1 rounded-md">
            <span>Today</span>
      ...
      {/* Hero Card: Collected Amount / Revenue at Risk (from screenshot) */}
      <div className="rzp-card p-6 shadow-xs bg-white border border-slate-200 rounded-xl">
      ...
      {/* 3 Secondary Cards Row (from screenshot: Refunds, Disputes, Failed/Recovered) */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        
        {/* Card 1: Revenue Recovered */}
        <div className="rzp-card rzp-card-hover p-5 border border-slate-200 rounded-xl flex flex-col justify-between">
          <div>
        ...
        {/* Card 3: Active Cases & Escalations */}
        <div className="rzp-card rzp-card-hover p-5 border border-slate-200 rounded-xl flex flex-col justify-between">
          <div>
            <div className="flex items-center justify-between text-xs font-bold text-slate-700">
      ...
      {/* Main Content Area Sub-Navigation Tabs (from screenshot) */}
      <div className="space-y-4 pt-2">
        ...
        {/* Cases Stream Table Container (from screenshot) */}
                  ...
                  <tr key={item.case_id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="py-3 px-4 text-right">
                      <button
                        ...
                        className="px-3 py-1 bg-slate-100 hover:bg-blue-50 text-blue-700 rounded-md text-xs font-bold border border-slate-200 transition-colors"
                      >
                        Inspect
                      </button>
                    </td>
      ...
      {/* Floating Action Button (Help & Support pill from screenshot) */}
      <div className="fixed bottom-5 right-5 z-40">
        <button
          onClick={onOpenSimulator}
          className="bg-[#0f172a] hover:bg-[#0c2340] text-white px-4 py-2.5 rounded-full shadow-2xl flex items-center space-x-2 text-xs font-bold border border-slate-700 transition-all transform hover:scale-105"
        >
          <Zap className="w-4 h-4 text-amber-400 fill-current" />
          <span>Help & Webhook Simulator</span>
        </button>
      </div>
